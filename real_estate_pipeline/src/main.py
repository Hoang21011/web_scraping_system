...
# Crawl dữ liệu về căn hộ
# Nếu hệ thống quét 1 trang mà toàn bộ id của căn nhà đã tồn tại, vòng lặp sẽ break
def scrape_properties(data_manager):
    page = 1
    total_properties_saved = 0
    logger.info("--- STARTING HTML PAGINATION CRAWLER ---")
    while True:
...
# Hệ thống gom tất cả các mã ID của cả dự án và phân khu con vào một danh sách, sau đó dùng list(set(target_ids)) để xóa bỏ các ID bị trùng lặp.
# Nếu lịch sử giá đã từng được lấy, hệ thống sẽ tiếp tục chạy, bỏ qua lịch sử giá đó.
# Nếu là id mới, hệ thống sẽ gọi API để lấy lịch sử gía và biểu đồ giá.
def fetch_projects_and_subdivisions(data_manager):
    # Fetch detailed projects and subdivisions.
    ...
    logger.info("--- STARTING FETCH DETAILED PROJECTS & SUBDIVISIONS ---")
    ...
    for base_proj in valid_projects:
        ...
        # Vì 1 dự án lớn cần có nhiều phân khu, nên phải tách từng phân khu ra
...
def fetch_project_pricing(data_manager, projects, subdivisions):
    # Fetch Price History and Price Chart for projects & subdivisions.
    logger.info("--- STARTING FETCH PROJECT PRICING ---")
